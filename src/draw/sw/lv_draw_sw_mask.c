...
LV_ATTRIBUTE_FAST_MEM static lv_draw_sw_mask_res_t lv_draw_mask_polygon(lv_opa_t * mask_buf, lv_coord_t abs_x,
                                                                        lv_coord_t abs_y, lv_coord_t len,
                                                                        lv_draw_sw_mask_polygon_param_t * param);
...
void lv_draw_sw_mask_free_param(void * p)
{
    ...
    else if(pdsc->type == LV_DRAW_SW_MASK_TYPE_POLYGON) {
...
void lv_draw_sw_mask_polygon_init(lv_draw_sw_mask_polygon_param_t * param, const lv_point_t * points,
                                  uint16_t point_cnt)
{
    ...
    param->dsc.cb = (lv_draw_sw_mask_xcb_t)lv_draw_mask_polygon;
    param->dsc.type = LV_DRAW_SW_MASK_TYPE_POLYGON;
}
                                                                        ...
                                                                        lv_draw_sw_mask_polygon_param_t * param)
{
    ...
    if(line_cnt != 2) return LV_DRAW_SW_MASK_RES_TRANSP;
    if(lines[0].p1.x > lines[1].p1.x || lines[0].p2.x > lines[1].p2.x) {
        tmp = lines[0];
        lines[0] = lines[1];
        lines[1] = tmp;
    }
    lv_draw_sw_mask_line_param_t line_p;
    lv_draw_sw_mask_line_points_init(&line_p, lines[0].p1.x, lines[0].p1.y, lines[0].p2.x, lines[0].p2.y,
    ...
    if(line_p.steep == 0 && line_p.flat) {
        ...
        lv_draw_sw_mask_free_param(&line_p);
        return LV_DRAW_SW_MASK_RES_CHANGED;
    }
    lv_draw_sw_mask_res_t res1 = lv_draw_mask_line(mask_buf, abs_x, abs_y, len, &line_p);
    lv_draw_sw_mask_free_param(&line_p);
    if(res1 == LV_DRAW_SW_MASK_RES_TRANSP) {
        return LV_DRAW_SW_MASK_RES_TRANSP;
    }
    lv_draw_sw_mask_line_points_init(&line_p, lines[1].p1.x, lines[1].p1.y, lines[1].p2.x, lines[1].p2.y,
    ...
    lv_draw_sw_mask_res_t res2 = lv_draw_mask_line(mask_buf, abs_x, abs_y, len, &line_p);
    lv_draw_sw_mask_free_param(&line_p);
    if(res2 == LV_DRAW_SW_MASK_RES_TRANSP) {
        return LV_DRAW_SW_MASK_RES_TRANSP;
    }
    if(res1 == LV_DRAW_SW_MASK_RES_CHANGED || res2 == LV_DRAW_SW_MASK_RES_CHANGED) return LV_DRAW_SW_MASK_RES_CHANGED;
    return res1;
}
/**
